...
public class TestBlockFromToEvent {
	...
	// This test is always returning True, even if I delete the custom
	// BlockFromToEventListener the plugin registered. I need to figure out how to
	// simulate the
	// water flowing into the torch in order to verify this is working. Calling the
	// BlockFromToEvent manually doesn't seem to do anything unfortunately
	@Test
	public void testBlockFromTo() {
		...
		HoneypotBlockManager.getInstance().createBlock(torch, "nothing");
		...
		Assertions.assertTrue(HoneypotBlockManager.getInstance().isHoneypotBlock(torch));
