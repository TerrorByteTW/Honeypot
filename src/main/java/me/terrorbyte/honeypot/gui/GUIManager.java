...
public class GUIManager{
	...
	private GUIPageButtonBuilder defaultPaginationButtonBuilder = (type, inventory) -> {
        switch (type) {
            case PREV_BUTTON:
                if (inventory.getCurrentPage() > 0) return new GUIButton(new GUIItemBuilder(Material.ARROW)
            ...
            case CURRENT_BUTTON:
                return new GUIButton(new GUIItemBuilder(Material.NAME_TAG)
            ...
            case NEXT_BUTTON:
                if (inventory.getCurrentPage() < inventory.getMaxPage() - 1) return new GUIButton(new GUIItemBuilder(Material.ARROW)
                        .name("&a&lNext Page \u2192")
                        .lore(
